	...
	with requests.Session() as s:
		...
		for title in soup.find_all('p', class_='result-info'):
			...
			if check_day > window and get_num_mtn == check_mtn:
				...
				title = get_link.text
				pattern = re.search(r'uplex', title)
				if pattern:
...
def write_info(url):
	...
	cycle(url)
	csv_file = open('denverApa.csv', 'w')
	csv_writer = csv.writer(csv_file)
	csv_writer.writerow(['City', 'URL', 'Title', 'DatePosted', 'Price', 'Email', 'Phone'])
	for link in links:
		csv_writer.writerow([city, link[0], link[1], link[2]])

#write_info('https://denver.craigslist.org/search/roo')
#time.sleep(120)
write_info('https://denver.craigslist.org/search/apa')
